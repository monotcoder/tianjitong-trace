...
import type { TraceData } from '@/mock/trace'

const props = defineProps<{ traceCode?: string }>()
...
const warning = computed(() => trace.value && trace.value.status !== 'published')
...
onMounted(async () => {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/trace/${props.traceCode}`)
    const json = await res.json()
    if (json.code !== 0 || !json.data) {
      error.value = json.message || '追溯码无效'
      return
    }
    trace.value = json.data
    openMap.value = Object.fromEntries(
      json.data.sections.map((s: any) => [s.key, s.defaultOpen]),
    )
  } catch (e) {
...
<template>
  <div class="mx-auto flex min-h-screen max-w-[480px] flex-col bg-muted/30">
    ...
    <div v-if="loading" class="flex flex-1 flex-col items-center justify-center gap-3 py-20">
      ...
      <span class="text-sm text-muted-foreground">正在查询追溯信息…</span>
    </div>
    ...
    <template v-else-if="trace">
      ...
      <section class="mx-4 mt-3 overflow-hidden rounded-2xl bg-background shadow-sm">
        <div class="aspect-[4/3] w-full bg-gradient-to-br from-primary/15 via-primary/5 to-background">
          <img v-if="trace.imageUrl" :src="trace.imageUrl" class="h-full w-full object-cover" alt="" />
        ...
        <div class="p-4">
          ...
          <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
            ...
            <div v-if="trace.produceDate">生产日期 · {{ trace.produceDate }}</div>
          </div>
          <div v-if="trace.origin" class="mt-1 text-sm text-muted-foreground">产地 · {{ trace.origin }}</div>
          <div v-if="trace.producer" class="mt-3 flex items-center gap-2 rounded-lg bg-primary/10 px-3 py-2 text-sm text-primary">
    ...
    <footer class="mt-6 flex flex-col items-center gap-1 px-4 pb-8 pt-4">
      ...
      <div v-if="traceCode" class="mt-1 text-[10px] text-muted-foreground/70">
        追溯码 · {{ traceCode }}
      </div>
    </footer>
  </div>
